...
namespace backend
{
  ...
  template <typename T>
  Info DenseVector<T>::allocate( const Index nvals )
  {
  ...
  template <typename T>
  Info DenseVector<T>::resize( const Index nvals )
  {
    T* h_tempVal = h_val_;
    T* d_tempVal = d_val_;
  ...
  template <typename T>
  Info DenseVector<T>::fill( const Index nvals )
  {
    for( Index i=0; i<nvals; i++ )
      h_val_[i] = i;
  ...
  template <typename T>
  const T& DenseVector<T>::operator[]( const Index ind )
  {
    gpuToCpu();
  ...
  template <typename T>
  inline Info DenseVector<T>::getNvals( Index& nvals ) const
  {
    nvals = nvals_;
    return GrB_SUCCESS;
  }
