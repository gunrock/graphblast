...
namespace backend
{
  ...
  class SparseMatrix
  {
    ...
    SparseMatrix( const Index nrows, const Index ncols, const Index nvals )
        : nrows_(nrows), ncols_(ncols), nvals_(nvals), ncapacity_(0), 
          nempty_(0),
          h_csrColInd_(NULL), h_csrRowPtr_(NULL), h_csrVal_(NULL),
          d_csrColInd_(NULL), d_csrRowPtr_(NULL), d_csrVal_(NULL), 
          buffer_(NULL),      need_update_(0) {}
    ...
    // Mutators
    // public method for setting nrows and ncols
    ...
    Info getVal( T& col_ind, const Index row_ptr ) const;
    Info getVal( T& col_ind, const Index offset, const Index row ) const;
  ...
  template <typename T>
  Info SparseMatrix<T>::nnew( const Index nrows, 
                              const Index ncols, 
                              const Index nvals )
  {
    nrows_ = nrows;
  ...
  template <typename T>
  Info SparseMatrix<T>::setNrows( const Index nrows )
  {
    nrows_ = nrows;
    return GrB_SUCCESS;
  }
  ...
  template <typename T>
  Info SparseMatrix<T>::setNcols( const Index ncols )
  {
    ncols_ = ncols;
    return GrB_SUCCESS;
  }
  ...
  template <typename T>
  Info SparseMatrix<T>::resize( const Index nrows, const Index ncols )
  {
    if( nrows<=nrows_ )
      nrows_ = nrows;
  ...
  template <typename T>
  template <typename U>
  Info SparseMatrix<T>::fill( const Index axis, 
                              const Index nvals,
                              const U     start )
  {
    Info err;
  ...
  template <typename T>
  template <typename U>
  Info SparseMatrix<T>::fillAscending( const Index axis, 
                                       const Index nvals,
                                       const U     start )
  {
    Info err;
  ...
  template <typename T>
  const T SparseMatrix<T>::operator[]( const Index ind )
  {
    gpuToCpu(true);
  ...
  template <typename T>
  inline Info SparseMatrix<T>::getNrows( Index& nrows ) const
  {
    nrows = nrows_;
    return GrB_SUCCESS;
  }
  ...
  template <typename T>
  inline Info SparseMatrix<T>::getNcols( Index& ncols ) const
  {
    ncols = ncols_;
    return GrB_SUCCESS;
  }
  ...
  template <typename T>
  inline Info SparseMatrix<T>::getNvals( Index& nvals ) const
  {
    nvals = nvals_;
    return GrB_SUCCESS;
  }
  ...
  // TODO update to GPU
  /*template <typename T>
  inline Info SparseMatrix<T>::getRowPtr( Index& row_ptr, 
                                   const Index row ) const
  {
    if( h_csrRowPtr_==NULL ) 
      return GrB_UNINITIALIZED_OBJECT;
    if( row>nrows_+1 ) 
      return GrB_INDEX_OUT_OF_BOUNDS;
    row_ptr = h_csrRowPtr_[ row ];
    return GrB_SUCCESS;
  }

  // TODO update to GPU
  template <typename T>
  inline Info SparseMatrix<T>::getColInd( Index& col_ind, 
                                   const Index row_ptr ) const
  {
    if( h_csrColInd_==NULL ) 
      return GrB_UNINITIALIZED_OBJECT;
    if( row_ptr>nvals_ ) 
      return GrB_INDEX_OUT_OF_BOUNDS;
    col_ind = h_csrColInd_[ row_ptr ];
    return GrB_SUCCESS;
  }
  ...
  inline Info SparseMatrix<T>::getColInd( Index& col_ind, 
                                   ...
                                   const Index row ) const
  {
    if( h_csrRowPtr_==NULL || h_csrColInd_==NULL ) 
      return GrB_UNINITIALIZED_OBJECT;
    if( row>nrows_+1 || offset>nvals_ ) 
      return GrB_INDEX_OUT_OF_BOUNDS;
    Index row_offset = h_csrRowPtr_[row] + offset;
    col_ind = h_csrColInd_[ row_offset ];
    return GrB_SUCCESS;
  }

  // TODO update to GPU
  template <typename T>
  inline Info SparseMatrix<T>::getVal( T&    col_val, 
                                const Index row_ptr ) const
  {
    if( h_csrColInd_==NULL ) 
      return GrB_UNINITIALIZED_OBJECT;
                                ...
                                const Index row ) const
  {
    ...
    if( row>nrows_+1 || offset>nvals_ ) 
      return GrB_INDEX_OUT_OF_BOUNDS;
    Index row_offset = h_csrRowPtr_[row] + offset;
    col_val = h_csrVal_[ row_offset ];
    return GrB_SUCCESS;
  }

  // TODO update to GPU
  template <typename T>
  Info SparseMatrix<T>::find( T& found, const Index target, const Index row )
      const
  {
    Index row_start = h_csrRowPtr_[row];
    Index row_end   = row_start+h_rowLength_[row];
    Index index     = binarySearch( h_csrColInd_, target, row_start, row_end );
    if( index!=-1 )
      found = h_csrVal_[index];
    else
      found = 0.0;
    return GrB_SUCCESS;
  }
  ...
  // TODO update to GPU
  template <typename T>
  Info SparseMatrix<T>::checkRowReduce( const T sum, const Index row ) const
  {
    Index row_start = h_csrRowPtr_[row];
    Index row_end   = row_start+h_rowLength_[row];
    T new_sum = 0;
    //printArray( "colind", h_csrColInd_+row_start, h_rowLength_[row] );
    //printArray( "val", h_csrVal_+row_start, h_rowLength_[row] );
    for( ; row_start<row_end; row_start++ )
    {
      new_sum += h_csrVal_[row_start];
    }
    //std::cout << "Comparing " << new_sum << " = " << sum << std::endl;
    assert( new_sum==sum );
    return GrB_SUCCESS;
  }*/
