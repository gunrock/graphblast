...
do
  if [ "$TEST" = "0" ] ; then
    ...
    bin/gpr --timing 0 --mxvmode 0 --niter 5 --max_niter 10 $DATA/$file/$file.mtx
  fi
done
...
do
  if [ "$TEST" = "1" ] ; then
    ...
    bin/gpr --timing 0 --mxvmode 0 --niter 5 --max_niter 10 $DATA2/$file.mtx
  fi
done
...
do
  if [ "$TEST" = "2" ] ; then
    ...
    bin/gpr --timing 0 --mxvmode 0 --niter 5 --max_niter 10 data/small/$file.mtx
  fi
done
