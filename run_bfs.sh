...
do
  if [ "$TEST" = "0" ] ; then
    bin/gbfs --timing 1 --earlyexit 1 --mxvmode 0 --struconly 1 --niter 0 --opreuse 1 $DATA/$file/$file.mtx
  fi
done
...
do
  if [ "$TEST" = "2" ] ; then
    bin/gbfs --timing 1 --earlyexit 1 --mxvmode 0 --struconly 1 --niter 0 --opreuse 1 data/small/$file.mtx
  fi
done
