...
int main(int argc, char** argv) {
  ...
  CpuTimer warmup;
  warmup.Start();
  graphblas::algorithm::gcCusparse(&v, &a, source, max_colors, &desc);
  warmup.Stop();

  std::vector<int> h_gc_gpu;
  CHECK(v.extractTuples(&h_gc_gpu, &nrows));
  graphblas::algorithm::verifyGc(&a, h_gc_gpu, true);
  ...
  for (int i = 0; i < niter; i++) {
    val = graphblas::algorithm::gcCusparse(&v, &a, source, max_colors, &desc);
    tight += val;
  }
  // cudaProfilerStop();
  vxm_gpu.Stop();
